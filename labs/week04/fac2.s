...
# ... place the parameter in $a0 and get the result from $v0 ...
	move $a0, $v0			# placing n in $a0
	jal fac					# jumps to fac	
	move $t0, $v0			# placing n in $t0
...
# ... use the value of n in $a0, place n! in $v0 ...
	...
	while:
		bgt $s0, $a0, end_while 	# end while loop when i ($s0) > n ($a0)
		mul $s1, $s1, $s0			# prod *= i
		addi $s0, $s0, 1			# i++
		j while

	end_while:
		move $v0, $s1				# value of n! stored in $v0
